...
export default function Auth() {
  ...
  const handleGithubCallback = async (code) => {
    setIsLoading(true);
    try {
      const res = await axiosInstance.post('/auth/github', { 
        code, 
      ...
      } else if (res.data.accountType === 'organization') {
        navigate('/dashboard');
      } else {
        navigate(`/user/${res.data.username || 'profile'}`); 
      }
    } catch (err) {
      setError(err.response?.data?.message || 'GitHub auth failed');
    } finally {
      setIsLoading(false);
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      setError('');
      try {
        const res = await axiosInstance.post('/auth/g-login', { 
        ...
        const detailedError = err.response?.data?.googleError ? JSON.stringify(err.response.data.googleError) : '';
        const msg = err.response?.data?.message || `Network Error: ${err.message}`;
        setError(msg + (detailedError ? ' ' + detailedError : ''));
      } finally {
        setIsLoading(false);
      }
    },
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 relative z-10">
        ...
        >
          ...
          <div className="bg-white/80 dark:bg-[#111522]/80 backdrop-blur-xl border border-authra-border-light dark:border-[#2A3155] rounded-3xl p-6 sm:p-8 shadow-[0_8px_30px_rgba(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.2)]">
            ...
            <button type="button" onClick={() => handleGoogleLogin()} className="w-full flex items-center justify-center gap-3 bg-white dark:bg-[#0D0F16] border border-authra-border-light dark:border-[#2A3155] text-authra-text-light dark:text-[#F5F8FF] font-medium rounded-xl p-3 hover:bg-black/5 dark:hover:bg-white/5 transition-all mb-3">
              <GoogleIcon />
              Continue with Google
            </button>

            <button type="button" onClick={handleGithubLogin} className="w-full flex items-center justify-center gap-3 bg-white dark:bg-[#0D0F16] border border-authra-border-light dark:border-[#2A3155] text-authra-text-light dark:text-[#F5F8FF] font-medium rounded-xl p-3 hover:bg-black/5 dark:hover:bg-white/5 transition-all">
              <LinkedinIcon className="w-5 h-5" /> {/* Note: Using LinkedinIcon here as placeholder, ideally add a GithubIcon SVG */}
