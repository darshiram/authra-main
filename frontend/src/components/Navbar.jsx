...
export default function Navbar() {
  ...
  if (!user) {
    ...
    navLinks = ['Admin Panel', 'Dashboard', 'My Profile', 'About'];
  } else if (user.accountType === 'organization') {
    navLinks = ['Dashboard', 'Public Profile', 'Pricing', 'About'];
  } else {
    navLinks = ['Dashboard', 'My Profile', 'About'];
  }
        ...
        >
          ...
          <div className="hidden md:flex items-center gap-6">
            {navLinks.map((link, index) => {
              const isRoute = ['Dashboard', 'Admin Panel', 'My Profile', 'Public Profile', 'Pricing', 'About', 'Features'].includes(link);
              const path = link === 'Dashboard' ? '/dashboard'
                : link === 'Admin Panel' ? '/admin'
                  : link === 'My Profile' ? `/user/${user?.username || user?.email?.split('@')[0] || 'profile'}`
                    : link === 'Public Profile' ? `/o/${user?.username || 'profile'}`
